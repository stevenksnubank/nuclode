...
# Cap namespace count per group to bound output size.
# At ~300 tokens per namespace, 15 ns ≈ 4500 output tokens — fits in 8K max_tokens.
_MAX_NAMESPACES_PER_GROUP = 15
...
def _split_oversized_groups(
    ...
    """Split groups that exceed input size or namespace count limits.

    Groups under both limits pass through unchanged. Oversized groups get
    ...
    for group in groups:
        ...
        if total_chars <= _MAX_SOURCE_CHARS_PER_GROUP and ns_count <= _MAX_NAMESPACES_PER_GROUP:
            result.append(group)
            continue

        # Split by whichever limit is tighter
        splits_by_chars = math.ceil(total_chars / _MAX_SOURCE_CHARS_PER_GROUP) if total_chars > _MAX_SOURCE_CHARS_PER_GROUP else 1
        splits_by_count = math.ceil(ns_count / _MAX_NAMESPACES_PER_GROUP) if ns_count > _MAX_NAMESPACES_PER_GROUP else 1
        num_splits = max(splits_by_chars, splits_by_count)
        chunk_size = math.ceil(ns_count / num_splits)
