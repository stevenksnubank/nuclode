...
## Instructions
Analyze these namespaces. You can see ALL the source code for this flow group.

For each namespace, identify:
- Its role in the flow (1-2 sentences)
- Its Diplomat layer classification
- Side effects (datomic, kafka, http, s3, etc.)
- Security notes (if any)

At the group level, identify:
- Bottlenecks (namespaces where multiple flows converge)
- Coupling issues (cross-domain dependencies, layer violations)
{security_instructions}

Do NOT include data_flow edges — dependency edges are derived from the code graph directly.
