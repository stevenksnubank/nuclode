...
def reduce_to_beads(
    ...
    All beads are stored in nuclode's output directory, never in the target project.
    Dependency links are derived from the code graph (groups), not sub-LM output.

    Args:
        analyses: List of validated flow-analysis dicts.
        db_path: Path to the beads database file (inside nuclode's output dir).
        groups: Flow groups from partition — used to derive dependency links.
    ...
    # db_path must already be initialized by the orchestrator before we get here
    ...
    for analysis in analyses:
        ...
        if tag_list:
            tag_bead(bead_id, tag_list, db_path=db_path)
            tags_applied += len(tag_list)

    # Link beads based on dependency graph (deterministic, not sub-LM derived)
    if groups:
        # Build namespace → flow_name lookup from analyses
        ns_to_flow: dict[str, str] = {}
        for analysis in analyses:
            for ns in analysis.get("namespaces", []):
                ns_to_flow[ns["name"]] = analysis["flow_name"]

        # Derive cross-group links from internal_deps
        for group in groups:
            flow_name = group.name
            bead_id = bead_ids.get(flow_name)
            if not bead_id:
                continue
            for from_ns, to_list in group.internal_deps.items():
                for to_ns in to_list:
                    target_flow = ns_to_flow.get(to_ns)
                    if target_flow and target_flow != flow_name:
                        target_bead_id = bead_ids.get(target_flow)
                        if target_bead_id:
                            link_beads(bead_id, target_bead_id, "depends-on", db_path=db_path)
                            links_created += 1
